...
ViewAccountsDialog::ViewAccountsDialog(LinkedList<User>* users,
                                       QWidget* parent)
    : QDialog(parent)
    , m_userList(users)
{
    ...
    for (Node<User>* cur = m_userList->getHead(); cur; cur = cur->next) {
        ...
        if (!u->isAdmin()) {
            ...
            ui.tableWidget->setItem(row, 3,
                                    new QTableWidgetItem(QString::fromStdString(u->getCardNumber())));
            ui.tableWidget->setItem(row, 4,
                                    new QTableWidgetItem(QString::fromStdString(u->getExpiryDate())));
            ui.tableWidget->setItem(row, 5,
                                    new QTableWidgetItem(QString::number(u->getBalance())));
            ++row;
        }
    }
}
